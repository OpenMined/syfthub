 ...
 *
 * Gradient ids are namespaced per-instance via useId(): with a shared id,
 * every `fill="url(#sharedId)"` resolves to whichever <linearGradient> is
 * first in the DOM, so the icon flattens to its solid base color the moment
 * that instance unmounts (e.g. when the thinking indicator disappears).
 */
export function OpenMinedIcon({ className }: Readonly<OpenMinedIconProps>): ReactElement {
  const uid = useId().replace(/:/g, '');
  const p = (n: number): string => `om-paint-${n}-${uid}`;

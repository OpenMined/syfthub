...
class TestDataSourceEndpointExecution:
    ...
    def test_datasource_query_returns_documents(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_datasource_empty_results(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_datasource_with_custom_request_params(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_datasource_response_model_validation(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
...
class TestModelEndpointExecution:
    ...
    def test_model_query_returns_response(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_model_with_conversation_history(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_model_response_has_unique_id(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_model_response_schema_validation(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
...
class TestMultipleEndpoints:
    ...
    def test_multiple_datasources(
        ...
        resp1 = client.post(
            "/api/v1/endpoints/ds-one/query",
            json={"messages": "query"},
            headers=auth_headers,
        )
        resp2 = client.post(
            "/api/v1/endpoints/ds-two/query",
            json={"messages": "query"},
            headers=auth_headers,
        )
...
class TestErrorHandling:
    ...
    def test_invalid_message_role_returns_422(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_missing_auth_header_returns_401(self, app: SyftAPI) -> None:
        """Test that missing Authorization header returns 401."""

        @app.datasource(slug="auth-test", name="Auth Test", description="Tests auth")
        async def auth_test(query: str) -> list[Document]:
            return []

        client = TestClient(app.get_app())
        response = client.post(
            "/api/v1/endpoints/auth-test/query",
            json={"messages": "test"},
            # No auth headers
        )
...
class TestUserContextInjection:
    """Tests for UserContext injection into endpoint handlers."""

    def test_handler_without_user_context_works(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
        """Test that handlers without UserContext param work (auth still happens)."""

        @app.datasource(
            slug="no-user",
            name="No User",
            description="Endpoint without UserContext",
        )
        async def no_user(query: str) -> list[Document]:
            return [Document(document_id="1", content=query, similarity_score=1.0)]

        client = TestClient(app.get_app())
        response = client.post(
            "/api/v1/endpoints/no-user/query",
            json={"messages": "hello"},
            headers=auth_headers,
        )

        assert response.status_code == 200
        assert response.json()["references"]["documents"][0]["content"] == "hello"

    def test_handler_with_user_context_receives_user(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
        """Test that handlers with UserContext param receive the verified user."""

        @app.datasource(
            slug="with-user",
            name="With User",
            description="Endpoint with UserContext",
        )
        async def with_user(query: str, user: UserContext) -> list[Document]:
            return [
                Document(
                    document_id="1",
                    content=f"User: {user.username}, email: {user.email}",
                    similarity_score=1.0,
                )
            ]
        ...
        client = TestClient(app.get_app())
        response = client.post(
            "/api/v1/endpoints/with-user/query",
            json={"messages": "hello"},
            headers=auth_headers,
        )

        assert response.status_code == 200
        content = response.json()["references"]["documents"][0]["content"]
        assert "User: testuser" in content
        assert "email: test@example.com" in content

    def test_model_handler_with_user_context(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
        """Test that model handlers with UserContext param receive the verified user."""

        @app.model(
            slug="model-with-user",
            name="Model With User",
            description="Model with UserContext",
        )
        async def model_with_user(messages: list[Message], user: UserContext) -> str:
            return f"Hello {user.username}!"

        client = TestClient(app.get_app())
        response = client.post(
            "/api/v1/endpoints/model-with-user/query",
            json={"messages": [{"role": "user", "content": "hi"}]},
            headers=auth_headers,
        )

        assert response.status_code == 200
        assert response.json()["summary"]["message"]["content"] == "Hello testuser!"


class TestLifecycleHooksIntegration:
    """Tests for lifecycle hooks with endpoint execution."""

    def test_startup_hook_runs_before_requests(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:
...
class TestMiddlewareIntegration:
    ...
    def test_custom_middleware_processes_requests(
        self, app: SyftAPI, auth_headers: dict[str, str]
    ) -> None:

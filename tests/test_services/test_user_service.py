...
class TestUserServiceChangePassword:
    ...
    def test_change_password_own_success(self, user_service, sample_user):
        """Test user changing their own password."""
        with (
            patch("syfthub.auth.security.verify_password", return_value=True),
            patch("syfthub.auth.security.hash_password", return_value="new_hash"),
            patch.object(
                user_service.user_repository, "get_by_id", return_value=sample_user
            ),
            patch.object(
                user_service.user_repository, "update_password", return_value=True
            ),
        ):
            result = user_service.change_password(
                1, "old_pass", "new_password", sample_user
            )
    ...
    def test_change_password_admin_for_other(
        ...
        with (
            ...
            result = user_service.change_password(
                1, "any_pass", "new_password", admin_user
            )

            assert result is True

    def test_change_password_permission_denied(self, user_service, sample_user):
        """Test permission denied when changing another user's password."""
        with pytest.raises(HTTPException) as exc_info:
            user_service.change_password(2, "old_pass", "new_password", sample_user)

        assert exc_info.value.status_code == 403

    def test_change_password_user_not_found(self, user_service, admin_user):
        """Test changing password for non-existent user."""
        with patch.object(user_service.user_repository, "get_by_id", return_value=None):
            with pytest.raises(HTTPException) as exc_info:
                user_service.change_password(
                    999, "old_pass", "new_password", admin_user
                )

            assert exc_info.value.status_code == 404

    def test_change_password_incorrect_old_password(self, user_service, sample_user):
        """Test changing password with incorrect old password."""
        with (
            patch("syfthub.auth.security.verify_password", return_value=False),
            patch.object(
                user_service.user_repository, "get_by_id", return_value=sample_user
            ),
        ):
            with pytest.raises(HTTPException) as exc_info:
                user_service.change_password(
                    1, "wrong_pass", "new_password", sample_user
                )

            assert exc_info.value.status_code == 400
            assert "Current password is incorrect" in str(exc_info.value.detail)

    def test_change_password_too_short(self, user_service, sample_user):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc_info:
                user_service.change_password(1, "old_pass", "short", sample_user)

            assert exc_info.value.status_code == 400
            assert "must be at least 8 characters" in str(exc_info.value.detail)

    def test_change_password_update_failed(self, user_service, sample_user):
        ...
        with (
            ...
            patch.object(
                user_service.user_repository, "update_password", return_value=False
            ),
        ):
            with pytest.raises(HTTPException) as exc_info:
                user_service.change_password(1, "old_pass", "new_password", sample_user)

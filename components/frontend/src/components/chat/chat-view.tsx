...
export function ChatView({
  ...
  const nextMessageIdReference = useRef(0);
  const createMessageId = useCallback(() => {
    nextMessageIdReference.current += 1;
    return `chat-message-${nextMessageIdReference.current.toString()}`;
  }, []);
  ...
  const initialQueryMessageIdReference = useRef<string | null>(
    initialQuery && !initialResult ? createMessageId() : null
  );
      ...
      return [
        {
          ...
          id: initialQueryMessageIdReference.current ?? createMessageId(),
          role: 'user' as const,
  ...
  const handleSubmit = useCallback(
    ...
    [messages, runPrecheckThenProceed, submitWorkflow, clearSuggestions, createMessageId]
  );

...
| **Endpoint health reporting** | The mechanism clients use to report endpoint liveness. Clients call `POST /endpoints/health` to report each owned endpoint's health with a TTL (sets per-endpoint `health_status`, `health_checked_at`, `health_ttl_seconds`); it also registers the owner's domain. The Health Monitor marks any endpoint without a fresh per-endpoint report as unhealthy. |
